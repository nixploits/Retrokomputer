  ...
  <div class="max-w-3xl mx-auto space-y-6">
    ...
    >
      <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800">
        <h3
          class="text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-wider flex items-center gap-2"
        >
      ...
      <div v-else class="p-6 space-y-4 text-xs font-mono">
        <div
          class="flex justify-between items-center py-2.5 border-b border-slate-100 dark:border-slate-800"
        >
        ...
        >
          ...
          <span class="font-bold text-rose-600 dark:text-rose-400">{{
            formatCurrency(stats.kerugian_inventaris)
          }}</span>
        </div>
        <div class="pt-4 flex justify-between items-center">
          <span class="font-bold text-sm uppercase tracking-wide text-slate-800 dark:text-slate-100"
            >Laba Bersih</span
          >
          <span
            class="font-bold text-lg"
            :class="
              stats.laba_bersih >= 0 ? 'text-retro-success' : 'text-rose-600 dark:text-rose-400'
            "
          >
        ...
        <div
          class="pt-3 text-[10px] text-center text-slate-400 dark:text-slate-500 border-t border-slate-100 dark:border-slate-800"
        >
          Periode: Bulan ini
        </div>
      </div>
